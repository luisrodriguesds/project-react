...
type SubmitButton = {
  onSubmit: (subtitle: string, description: string) => void;
  text: string;
};
type GenericProps = {
  title: string;
  firstInput: string;
  img: string;
  secondInput: TextDisplay;
  button: SubmitButton;
};

const GenericCard: FunctionComponent<GenericProps> = ({
  children,
  title,
  firstInput,
  img,
  ...
  return (
    <Card style={{ background: "#ecd3d3" }}>
      ...
      <CardBody>
        ...
        {children}
        <Button onClick={() => button.onSubmit(sub, desc)}>
          {button.text}
        </Button>
      </CardBody>
    </Card>
